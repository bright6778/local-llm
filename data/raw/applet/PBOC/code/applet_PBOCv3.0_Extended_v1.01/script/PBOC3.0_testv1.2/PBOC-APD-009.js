/*******************************************
 Test Name  : Read Other Application Data (APD 009)
 ...
 for(i = 0 ; i < AFL.length ; i += 8)
{
	...
	for(j = startRecordNumber; j <= endRecordNumber; j++)
	{
		...
		AUC_Length = lookup_BER_TLV(recordData, "9F07", RETURN_LENGTH);
		AUC_value = lookup_BER_TLV(recordData, "9F07", RETURN_VALUE);
		  
		if(AUC_value!= ''){
			if(AUC_Length != '02') error ('length is missing');
			if(AUC_value != 'FFC0' ) error ('data is missing');
				}						
		 	}
		}
