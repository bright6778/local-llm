/*******************************************
 Test Name  : Read Other Application Data (APD 022)
 Objective  : Verify the UCOL is returned in response to a READ RECORD command.
*******************************************/
 ...
 for(i = 0 ; i < AFL.length ; i += 8)
{
	...
	for(j = startRecordNumber; j <= endRecordNumber; j++)
	{
		...
		UCOL_value = lookup_BER_TLV(recordData, "9F23", RETURN_VALUE);
		  
		
		
		if(UCOL_value != ''){
				if(UCOL_value!= '07') error ('data is missing');
				}						
		 	}
		}