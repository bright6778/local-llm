/*******************************************
 Test Name  : Read Other Application Data (APD 007)
 ...
 for(i = 0 ; i < AFL.length ; i += 8)
{
	...
	for(j = startRecordNumber; j <= endRecordNumber; j++)
	{
		...
		Application_PAN_length = lookup_BER_TLV(recordData, "5A", RETURN_LENGTH);
		application_PAN_value = lookup_BER_TLV(recordData, "5A", RETURN_VALUE);
		Track2 = lookup_BER_TLV(recordData, "57", RETURN_VALUE);
	  
		if(application_PAN_value!= ''){
			if(Application_PAN_length != '08') error ('length is missing');
			if(application_PAN_value != '4761739001010010') error ('data is missing');
				}                        
