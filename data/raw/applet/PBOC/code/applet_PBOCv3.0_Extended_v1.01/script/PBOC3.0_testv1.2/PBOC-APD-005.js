/*******************************************
 Test Name  : Read Other Application Data (APD 005)
 Objective  : Verify the Application Effective Date returned in response to a READ
RECORD command.
*******************************************/
...
var application_effectvie_date = '';
 for(i = 0 ; i < AFL.length ; i += 8)
{
	...
	for(j = startRecordNumber; j <= endRecordNumber; j++)
	{
		...
		application_effectvie_date = lookup_BER_TLV(recordData, "5F25", RETURN_VALUE);
		print('** application_effectvie_date : ' + application_effectvie_date);
		if(application_effectvie_date != '' && application_effectvie_date != '950701')error('data is missing');
	}
}
