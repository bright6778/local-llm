/*******************************************
 Test Name  : Read Other Application Data (APD 018)
 Objective  : Verify the Issuer Action Code-Online returned in response to a READ RECORD command.
*******************************************/
 ...
 for(i = 0 ; i < AFL.length ; i += 8)
{
	...
	for(j = startRecordNumber; j <= endRecordNumber; j++)
	{
		...
		IACD_length = lookup_BER_TLV(recordData, "9F0F", RETURN_LENGTH);
		IACD_value = lookup_BER_TLV(recordData, "9F0F", RETURN_VALUE);
		  
		
		
		if(IACD_value != ''){
			if(IACD_length != '05') error ('length is missing');
			if(IACD_value != 'F020049800') error ('data is missing');
