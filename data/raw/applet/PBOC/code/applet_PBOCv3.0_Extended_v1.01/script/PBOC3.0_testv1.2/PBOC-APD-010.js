/*******************************************
 Test Name  : Read Other Application Data (APD 010)
 Objective  : Verify the Application Version Number returned in response to a READ RECORD command
*******************************************/
 ...
 for(i = 0 ; i < AFL.length ; i += 8)
{
	...
	for(j = startRecordNumber; j <= endRecordNumber; j++)
	{
		...
		AVN_length = lookup_BER_TLV(recordData, "9F08", RETURN_LENGTH);
		AVN_value = lookup_BER_TLV(recordData, "9F08", RETURN_VALUE);
		  
		if(AVN_value != ''){
			if(AVN_length != '02') error ('length is missing');
			if(AVN_value != '0030' ) error ('data is missing');
