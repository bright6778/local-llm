/*******************************************
 Test Name  : Read Other Application Data (APD 017)
 Objective  : Verify the Issuer Action Code-Denial returned in response to a READ
RECORD command.
*******************************************/
 ...
 for(i = 0 ; i < AFL.length ; i += 8)
{
	...
	for(j = startRecordNumber; j <= endRecordNumber; j++)
	{
		...
		IACD_length = lookup_BER_TLV(recordData, "9F0E", RETURN_LENGTH);
		IACD_value = lookup_BER_TLV(recordData, "9F0E", RETURN_VALUE);
		  
		
		
		if(IACD_value != ''){
			if(IACD_length != '05') error ('length is missing');
			if(IACD_value != '0050880000') error ('data is missing');
