 ...
 * author: Junghwan Kim
   update: mllee
 * refernce: PBOC 3.0 Perso V1.1.1
 * description: select_image() -> init_image() -> store_data()
 */

include("..//_COMMON//_IMAGE_CONF.js");
...
function init_crypto() {	
	...
	if (issuer_key == CRYPTO_RSA_1024) {
		...
		SSAD = _ISSUER_PK_CERTIFICATE_1024; // 수정!!
	} else if (issuer_key == CRYPTO_RSA_1152) {
		CA_PK_INDEX = _CA_PK_INDEX_1152;
		ISSUER_PK_REMAINDER = _ISSUER_PK_REMAINDER_1152;
		ISSUER_PK_EXPONENT = _ISSUER_PK_EXPONENT_1152;
		ISSUER_PK_CERTIFICATE = _ISSUER_PK_CERTIFICATE_1152;
		IC_PK_CERTIFICATE = _IC_PK_CERTIFICATE_1152;
		SSAD = _ISSUER_PK_CERTIFICATE_1152; // 수정!!
		...
		SSAD = _ISSUER_PK_CERTIFICATE_1408; // 수정!!
	} else if (issuer_key == CRYPTO_RSA_1984) {
		CA_PK_INDEX = _CA_PK_INDEX_1984;
		ISSUER_PK_REMAINDER = _ISSUER_PK_REMAINDER_1984;
		ISSUER_PK_EXPONENT = _ISSUER_PK_EXPONENT_1984;
		ISSUER_PK_CERTIFICATE = _ISSUER_PK_CERTIFICATE_1984;
		IC_PK_CERTIFICATE = _IC_PK_CERTIFICATE_1984;
		SSAD = _ISSUER_PK_CERTIFICATE_1984; // 수정!!
	} 
		
	// check IC key length
	if (ic_key == CRYPTO_RSA_768) {
		IC_PK_EXPONENT = _E_768;
		IC_PK_REMAINDER = _IC_PK_REMAINDER_768;
		D = _D_768;
		N = _N_768;
		ONE_DEVIDE_Q_MOD_P = _ONE_DEVIDE_Q_MOD_P_768;
		D_MOD_Q_MINUS_ONE = _D_MOD_Q_MINUS_ONE_768;
		D_MOD_P_MINUS_ONE = _D_MOD_P_MINUS_ONE_768;
		Q = _Q_768;
		P = _P_768;
	}
	if (ic_key == CRYPTO_RSA_1024) {
		IC_PK_EXPONENT = _E_1024;
		...
		Q = _Q_1024;
		P = _P_1024;
	} else if (ic_key == CRYPTO_RSA_1152) {
		IC_PK_EXPONENT = _E_1152;
		IC_PK_REMAINDER = _IC_PK_REMAINDER_1152;
		D = _D_1152;
		N = _N_1152;
		ONE_DEVIDE_Q_MOD_P = _ONE_DEVIDE_Q_MOD_P_1152;
		D_MOD_Q_MINUS_ONE = _D_MOD_Q_MINUS_ONE_1152;
		D_MOD_P_MINUS_ONE = _D_MOD_P_MINUS_ONE_1152;
		Q = _Q_1152;
		P = _P_1152;
	}  else if (ic_key == CRYPTO_RSA_1280) {
		IC_PK_EXPONENT = _E_1280;
		IC_PK_REMAINDER = _IC_PK_REMAINDER_1280;
		D = _D_1280;
		N = _N_1280;
		ONE_DEVIDE_Q_MOD_P = _ONE_DEVIDE_Q_MOD_P_1280;
		D_MOD_Q_MINUS_ONE = _D_MOD_Q_MINUS_ONE_1280;
		D_MOD_P_MINUS_ONE = _D_MOD_P_MINUS_ONE_1280;
		Q = _Q_1280;
		P = _P_1280;
	} else if (ic_key == CRYPTO_RSA_1408) {
		IC_PK_EXPONENT = _E_1408;
		...
		Q = _Q_1408;
		P = _P_1408;
	} /*else if (ic_key == CRYPTO_RSA_1976) {
		IC_PK_EXPONENT = _E_1976;
		//IC_PK_REMAINDER = _IC_PK_REMAINDER_1976;
		D = _D_1976;
		N = _N_1976;
		ONE_DEVIDE_Q_MOD_P = _ONE_DEVIDE_Q_MOD_P_1976;
		D_MOD_Q_MINUS_ONE = _D_MOD_Q_MINUS_ONE_1976;
		D_MOD_P_MINUS_ONE = _D_MOD_P_MINUS_ONE_1976;
		Q = _Q_1976;
		P = _P_1976;
	}*/ else if (ic_key == CRYPTO_RSA_1984) {
		IC_PK_EXPONENT = _E_1984;
		IC_PK_REMAINDER = _IC_PK_REMAINDER_1984;
		D = _D_1984;
		N = _N_1984;
		ONE_DEVIDE_Q_MOD_P = _ONE_DEVIDE_Q_MOD_P_1984;
		D_MOD_Q_MINUS_ONE = _D_MOD_Q_MINUS_ONE_1984;
		D_MOD_P_MINUS_ONE = _D_MOD_P_MINUS_ONE_1984;
		Q = _Q_1984;
		P = _P_1984;
	} else if (ic_key == null) {
		IC_PK_EXPONENT = null;
		IC_PK_REMAINDER = null;
		D = null;
		N = null;
		ONE_DEVIDE_Q_MOD_P = null;
		D_MOD_Q_MINUS_ONE = null;
		D_MOD_P_MINUS_ONE = null;
		Q = null;
		P = null;
	}
	return;
...
function init_image(sel) {
	...
	if(app_level == APP_EXTENDED_LEVEL && currency == DUAL_CURRENCY){
		data = data.substring(0, 62) + '80' + data.substring(64, data.length) //CUP2013 (extended secondary currency)

		//var data =	string_length(PBOC_PACKAGE_ID.length/2)+PBOC_PACKAGE_ID + string_length(PBOC_APP_ID.length/2)+ PBOC_APP_ID+ string_length(id.length/2) + id + '01100BC909000000800001B0000000'; 
	}
  print('\n* Install PBOC'); // package applet instance parameter
	...
	if (sel == PSE)
		include("..//_COMMON//_IMAGE_PSE.js");
	else if (sel == PPSE)
		include("..//_COMMON//_IMAGE_PPSE.js");
	else if (sel == DUAL) {
		include("..//_COMMON//_IMAGE_PSE.js");
		include("..//_COMMON//_IMAGE_PPSE.js");
	}	
...
function store_data(level, currency) {
	...
	if (level >= APP_QPBOC_LEVEL) {
		...
		if (ic_key != CRYPTO_RSA_1984 && ic_key != CRYPTO_RSA_1152 && ic_key != CRYPTO_RSA_1408 && ic_key != CRYPTO_RSA_1280 && issuer_key != CRYPTO_RSA_1984)//youmeng20130904
			{
				data += '94' + string_length(AFL_QPBOC.length/2) + AFL_QPBOC
			}
		// 9F4B, 9F5D auto perso in applet 
		else if  (ic_key == CRYPTO_RSA_1984)//youmeng 20131127
			{
				data += '94' + string_length(AFL_QPBOC_1984_KEY.length/2) + AFL_QPBOC_1984_KEY
			}
		else
			{
				 data += '94' + string_length(AFL_QPBOC_LONG_KEY.length/2) + AFL_QPBOC_LONG_KEY
   		}
		/**/
	...
	if (ic_key == CRYPTO_RSA_1984 || ic_key == CRYPTO_RSA_1408 || ic_key == CRYPTO_RSA_1152 || ic_key == CRYPTO_RSA_1280 || issuer_key == CRYPTO_RSA_1984)	{//youmeng20130903
		// DGI 0102
		if (level >= APP_QPBOC_LEVEL) {
			print('\n* DGI 0102 (AFL: QPBOC ~ EXTENDED)');
			
			data ='8F' + string_length(CA_PK_INDEX.length/2) + CA_PK_INDEX
	...
	if (level >= APP_DC_LEVEL) {
		...
		if (ic_key != CRYPTO_RSA_1984 && ic_key != CRYPTO_RSA_1152 && ic_key != CRYPTO_RSA_1408 && ic_key != CRYPTO_RSA_1280)	{
			if (issuer_key != CRYPTO_RSA_1984)
			{
			data += '8F' + string_length(CA_PK_INDEX.length/2) + CA_PK_INDEX
					+ '9F32' + string_length(ISSUER_PK_EXPONENT.length/2) + ISSUER_PK_EXPONENT
					+ '92' + string_length(ISSUER_PK_REMAINDER.length/2) + ISSUER_PK_REMAINDER
	...
	if (level >= APP_DC_LEVEL) {
		...
		if (ic_key != CRYPTO_RSA_1984 && ic_key != CRYPTO_RSA_1152 && ic_key != CRYPTO_RSA_1408 && ic_key != CRYPTO_RSA_1280)	{	
			if (issuer_key != CRYPTO_RSA_1984)	
			{
			data += '9F0D' + string_length(EC_IAC_DEFAULT.length/2) + EC_IAC_DEFAULT
					+ '9F0E' + string_length(EC_IAC_DENIAL.length/2) + EC_IAC_DENIAL
					+ '9F0F' + string_length(EC_IAC_ONLINE.length/2) + EC_IAC_ONLINE
				}	
	...
	if (level >= APP_DC_LEVEL) {
		...
		if (img_num== 30 || img_num== 51){
			data  += '5F25' + string_length(APP_EFFECTIVE_DATE.length/2) + APP_EFFECTIVE_DATE
		}
		
		if (level >= APP_EC_LEVEL)	{//YOUMENG20130903
		    if	(ic_key != CRYPTO_RSA_1984 && ic_key != CRYPTO_RSA_1152 && ic_key != CRYPTO_RSA_1408 && ic_key != CRYPTO_RSA_1280)
		        {
		        	if (issuer_key != CRYPTO_RSA_1984)
		        	{
	...
	if (level >= APP_QPBOC_LEVEL) {
		...
		// DGI 0304
	if (level >= APP_EC_LEVEL) {//youmeng 20130902
		print('\n* DGI 0304 (AFL: EC )');
		
		data = '9F74' + string_length(EC_ISSUER_AUTH_CODE.length/2) + EC_ISSUER_AUTH_CODE
					
		send_command('00', '0304', data, '70', '00');
	}
	// DGI 0401
	if (level >= APP_DC_LEVEL) {
	...
	if (level >= APP_DC_LEVEL) {
		...
		ic_key = null;//for KONA232S test sunsw 20140710 
		if (ic_key != null) {
	...
	if (level == APP_EC_LEVEL) {
		...
		data += 'DF4F' + string_length(EC_LOG_FORMAT.length/2) + EC_LOG_FORMAT
		//data += '9F77' + string_length(QPBOC_EC_BALANCE_LIMIT.length/2) + QPBOC_EC_BALANCE_LIMIT
	...
	if (level == APP_EXTENDED_LEVEL) {
		...
		if(currency == DUAL_CURRENCY){
			if (EXTENDED_STEP_BY_STEP_REDUCTION_LIMIT_SA != null)
				data += 'DF82' + string_length(EXTENDED_STEP_BY_STEP_REDUCTION_LIMIT_SA.length/2) + EXTENDED_STEP_BY_STEP_REDUCTION_LIMIT_SA;
			if (EXTENDED_STEP_BY_STEP_REDUCTION_AMOUT_SA != null)
				data += 'DF83' + string_length(EXTENDED_STEP_BY_STEP_REDUCTION_AMOUT_SA.length/2) + EXTENDED_STEP_BY_STEP_REDUCTION_AMOUT_SA;
		}
	}
