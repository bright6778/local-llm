...
function select_PBOC()
{
	...
	//Value_DF61= lookup_BER_TLV(value_BF0C, "DF61", RETURN_VALUE);
...
function make_PDOL_Data(cdata){
	...
	if(cdata == '01'){
		...
		print('* Terminal_Transaction_Qualifiers: ' + Terminal_Transaction_Qualifiers);
		print('* Amount_Authorised: ' + Amount_Authorised);
		print('* Amount_Other: ' + Amount_Other);
		print('* Terminal_Country_Code: ' + Terminal_Country_Code);
		print('* TVR: ' + TVR);
		print('* Transaction_Currency_Code: ' + Transaction_Currency_Code);
		print('* Transaction_Date: ' + Transaction_Date);
		print('* Transaction_Type: ' + Transaction_Type);
		print('* Unpredictable_Number: ' + Unpredictable_Number + '\n');
	}
	else if(cdata == '11'){
		if(PDOL.length == 24)
...
function send_GPO(cdata)
{
	...
	if((TTQ_BYTE1 & BIT_MASK_6) == BIT_MASK_6 || (TTQ_BYTE1 & BIT_MASK_8) == BIT_MASK_8 || (TTQ_BYTE1 & BIT_MASK_7) == BIT_MASK_7){
		is_qPBOC = true;
		print('* qPBOC GPO Start');
		PDOL_Data = make_PDOL_Data(cdata);
	} else {
		is_qPBOC = false;
		print('* PBOC GPO Start\n');
		PDOL_Data = cdata; 
	}
...
//	print('*************************************************');
	...
	if(response.substr(0, 2) == "80"){
		...
		print("    Tag 82 : AIP, Len : " + lookup_BER_TLV(value_77, "82", RETURN_LENGTH) + ", Value : " + AIP);
		print("    Tag 9F36 : ATC, Len : " + lookup_BER_TLV(value_77, "9F36", RETURN_LENGTH) + ", Value : " + ATC);
		print("    Tag 57 : Track 2 Equivalent Data, Len : " + lookup_BER_TLV(value_77, "57", RETURN_LENGTH) + ", Value : " + Track2_Equivalent_Data);
		print("    Tag 9F10 : Issuer Application Data, Len : " + lookup_BER_TLV(value_77, "9F10", RETURN_LENGTH) + ", Value : " + Issuer_Application_Data);
		print("    Tag 9F26 : Application Cryptogram, Len : " + lookup_BER_TLV(value_77, "9F26", RETURN_LENGTH) + ", Value : " + AC);

		print('\n-- conditional data elements');
		print("    Tag 5F34 : Application PAN Sequence Number, Len : " + lookup_BER_TLV(value_77, "5F34", RETURN_LENGTH) + ", Value : " + lookup_BER_TLV(value_77, "5F34", RETURN_VALUE));
		print("    Tag 9F6C : Card Transaction Qualifier, Len : " + lookup_BER_TLV(value_77, "9F6C", RETURN_LENGTH) + ", Value : " + lookup_BER_TLV(value_77, "9F6C", RETURN_VALUE));
		print("    Tag 9F5D : Available Offline Spending Amount, Len : " + lookup_BER_TLV(value_77, "9F5D", RETURN_LENGTH) + ", Value : " + lookup_BER_TLV(value_77, "9F5D", RETURN_VALUE));

		print('\n-- optional data element');
		print("    Tag 5F20 : Cardholder Name, Len : " + lookup_BER_TLV(value_77, "5F20", RETURN_LENGTH) + ", Value : " + lookup_BER_TLV(value_77, "5F20", RETURN_VALUE));
...
function send_Verify_withOption(p2, cdata, Option)
{
	...
	else if(p2 == "88")
	{
		...
		recordData = lookup_BER_TLV(send_ReadRecord(2,7), "70", RETURN_VALUE, script);
		var ICC_PIN_Certificate = lookup_BER_TLV(recordData, "9F2D", RETURN_VALUE, script);
		
		//ICC PIN Certificate가 없는 경우 ICC PK Modulus, Exponent를 사용
		if(ICC_PIN_Certificate == "")
		{
			recordData = lookup_BER_TLV(send_ReadRecord(2,5), "70", RETURN_VALUE, script);
			ICC_PIN_Encipherment_PK_Exponent = lookup_BER_TLV(recordData, "9F47", RETURN_VALUE, script);
			ICC_PIN_Encipherment_PK_Modulus = retrieve_ICC_PK_Modulus(issuer_PK_Modulus);
		}
		else
		{
			recordData = lookup_BER_TLV(send_ReadRecord(2,8), "70", RETURN_VALUE, script);
		  	ICC_PIN_Encipherment_PK_Exponent = lookup_BER_TLV(recordData, "9F2E", RETURN_VALUE, script);
...
function send_ExternalAuth_Invalid()
{
	...
	print('*******************************************');
	return send('008200000A'+ '112233445566778899AA 00');
}

function send_ExternalAuth_InvalidARPC()
{
	print('\n*******************************************');
	print('* External Authentication with invalid ARPC');
	print('*******************************************');
	var invalidARPC = tdes_cbc(d1, sessionDEA_A + sessionDEA_B);
	return send('008200000A'+ '1122334455667788' +  ARC_00 + '00');
}
...
{
	ARPC = calc_ARPC(ARC);
	var len = ARPC.length + ARC.length;
	return send('00820000' + toHex(len/2) + ARPC +  ARC);
}

function send_ExternalAuth_ARC_InvalidARPC(ARC)
{
	print('\n*******************************************');
	print('* ExternalAuth ARC + InvalidARPC');
	print('*******************************************');
	ARPC = '4032541ae11e32a0'; //Invalid ARPC
	var len = ARPC.length + ARC.length;
	return send('00820000' + toHex(len/2) + ARPC +  ARC);
}
...
function send_CardBlock_Invalid()
{
	print('\n*******************************************');
	print("* Card Block Invalid MAC");
	print('*******************************************');
	mac = calc_MAC_CBK();
	return send("8416000004" +  '11223344');
}

function send_CardBlock_noMAC()
{
	print('\n*******************************************');
	print("* Card Block without MAC");
	print('*******************************************');
	mac = calc_MAC_CBK();
	return send("8416000000");
}
...
function send_GEN_AC_1(p1, cdata)
{
	...
	if(getSW() == "9000")
	{
		...
		//validate_AC(AC); //YOUMENG 20130628
	}
...
function check_tags_in_response(response, tags)
{
	var data = lookup_BER_TLV(response, response.substring(0,2), RETURN_VALUE);
	...
	for (var i = 0; i<tags.length; i++) {
		if (lookup_BER_TLV(data, tags[i], RETURN_VALUE) != '')
...
function check_no_tags_in_response(response, tags)
{	
	var data = lookup_BER_TLV(response, response.substring(0,2), RETURN_VALUE);
...
function parse_InternalAuth_Response(response)
{
	...
	var recordData = lookup_BER_TLV(send_ReadRecord(2,2), "70", RETURN_VALUE);
...
function validate_AC(AC)
{
	...
	if(is_qPBOC == false){
		...
		switch(CVN){
		case '01' :
			sourceData = Amount_Authorised + Amount_Other + Terminal_Country_Code + TVR +
						 Transaction_Currency_Code + Transaction_Date + Transaction_Type + 
						 Unpredictable_Number + AIP + ATC + CVR.substring(0, 8);	
			break;
...
function validate_IAD_MAC(data)
{
        ...
        MAC = data.substring(data.length - 8, data.length);
        
        check_ValidATC();
        
        if (ID == '01') {
        	EC_BALANCE = send_Command_CorrectedLe("80 CA" + '9F79' + "00");
        ...
        calculated_MAC = ftdesMAC(sourceData + PAD, sessionDEA_A + sessionDEA_B);
        ...
        print("* Source Data : " + sourceData);
        print("* ATC : " + ATC);
        print("* Unique DEA Key : " + uniqueMAC_A.toUpperCase() + uniqueMAC_B.toUpperCase());
        print("* Session Key : " + sessionDEA_A.toUpperCase() + sessionDEA_B.toUpperCase());
        print("***********************************************");
        print("* PASS MAC : " + MAC);
        print("* TEST MAC : " + calculated_MAC.toUpperCase().substring(0, 8));
            
        if(calculated_MAC.toUpperCase().substring(0, 8) != MAC)
...
function retrieve_ICC_PIN_Encipherment_PK_Modulus(issuer_PK_Modulus)
{
	...
	recordData = lookup_BER_TLV(send_ReadRecord(2,7), "70", RETURN_VALUE, script);
	ICC_PIN_Certificate = lookup_BER_TLV(recordData, "9F2D", RETURN_VALUE, script);
	...
	recordData = lookup_BER_TLV(send_ReadRecord(2,2), "70", RETURN_VALUE, script);
	issuer_PK_Exponent = lookup_BER_TLV(recordData, "9F32", RETURN_VALUE, script);
	...
	recordData = lookup_BER_TLV(send_ReadRecord(2,8), "70", RETURN_VALUE, script);
	ICC_PK_Reminder = lookup_BER_TLV(recordData, "9F2F", RETURN_VALUE, script);
...
function retrieve_Issuer_PK_Modulus(CA_PK_Modulus, CA_PK_Exponent)
{
	...
	recordData = lookup_BER_TLV(send_ReadRecord(2,3), "70", RETURN_VALUE);
...
function validate_ICC_PK_Recovered(ICC_PK_Recovered, ICC_PK_Reminder, ICC_PK_Exponent, PAN)
{
	...
	//parse and calculate hash
	hashResult = ICC_PK_Recovered.substr(ICC_PK_Recovered.length - 42, 40);
	msg1 = ICC_PK_Recovered.substr(2, ICC_PK_Recovered.length - 44);
	//msg2 = ICC_PK_Reminder + ICC_PK_Exponent + retrieve_StaticData(AFL, AIP);
	//msg2 = ICC_PK_Reminder + ICC_PK_Exponent + sad + AIP;
	var sad = lookup_SAD();
	msg2 = ICC_PK_Reminder + ICC_PK_Exponent + sad;// + AIP;
	hash = messageDigest_SHA1(msg1 + msg2).toUpperCase();
	
	//print("* - Data to be hashed : " + msg1 + msg2);	
	print("* - Full_ICC_PK_Recovered : " + ICC_PK_Recovered);
	print("* - ICC_PK_Recovered : " + msg1);
	print("* - ICC_PK_Remainder : " + ICC_PK_Reminder);
	print("* - ICC_PK_Exponent : " + ICC_PK_Exponent);
	print("* - SAD : " + sad);
	print("* - AIP : " + AIP);
...
function validate_SDAD_forDDA(decrypted_SDAD)
{
	...
	print("* - Hash Result : " + hashResult); 
	if(hash != hashResult)
		error("* - Invalid Transaction Data Hash Code");
	else 
		print("* - Hash Result Validation...PASS");
	print('************************************************************************');
}
...
function validate_SDAD_for_fDDA(encrypted_SDAD)
{
	var msg1, msg2, hash, hashResult;
	var issuer_PK_Modulus;
	switch(PKI){
	case '80' :
		issuer_PK_Modulus = retrieve_Issuer_PK_Modulus(CA_PK_Modulus_128, CA_PK_Exponent_128); break;
	case '81' :
		issuer_PK_Modulus = retrieve_Issuer_PK_Modulus(CA_PK_Modulus_144, CA_PK_Exponent_144); break;
	case '82' :
		issuer_PK_Modulus = retrieve_Issuer_PK_Modulus(CA_PK_Modulus_160, CA_PK_Exponent_160); break;		
	}
	...
	print('*************************************************');
	if(decrypted_SDAD.substring(0,2) != "6A")
		error(" Recovered Data Validation...FAIL");
	else
		print("* Recovered Data Validation...PASS");
		
	if(decrypted_SDAD.substring(2,4) != "05")
		error(" Signed Data Format Format Validation...FAIL");
	else
		print("* Signed Data Format Format Validation...PASS");
		
	if(decrypted_SDAD.substring(decrypted_SDAD.length -2 ) != "BC")
		error(" Recovered Data Tailer Validation...FAIL");
	else
		print("* Recovered Data Tailer Validation...PASS");
		
	//Decrypted Signed Dynamic Application Data의 Hash Result 검사
	print('\n**********************************************************************');
	print("* Validate Hash Result in decrypted Signed Dynamic Application Data");
	print('************************************************************************');
	msg1 = decrypted_SDAD.substring(2, decrypted_SDAD.length - 42);
	//-> msg2 for fDDA 1.0
	//msg2 = Unpredictable_Number + Amount_Authorised + Transaction_Currency_Code + Card_Authentication_Related_Data; 
	//-> msg2 for fDDA 0.0
	msg2 = Unpredictable_Number;
	
	hash = messageDigest_SHA1(msg1 + msg2).toUpperCase();
	hashResult = decrypted_SDAD.substring(decrypted_SDAD.length - 42, decrypted_SDAD.length - 2); 
	
	print("* Data to be hashed : " + msg1 + msg2);	
	print("* Pass Hash : " + hashResult); 
	print("* Test Hash : " + hash);
	
	if(hash != hashResult)
		error(" Hash Result Validation...FAIL");
	else 
		print("* Hash Result Validation...PASS");
}
...
{
	var msg1, msg2, hash, hashResult, ICC_Dynamic_Data, ICC_Dynamic_Data_Length ;
	...
	msg1 = GEN_AC_Sequence == GEN_AC_1 ?  CDOL1_Data_for_Hash : CDOL1_Data_for_Hash + CDOL2_Data_for_Hash;
	...
	msg1 = msg1 + "9F27" + lookup_BER_TLV(value_77, "9F27", RETURN_LENGTH) + lookup_BER_TLV(value_77, "9F27", RETURN_VALUE)
							...
							//+ "9F4B" + lookup_BER_TLV(value_77, "9F4B", RETURN_LENGTH) + lookup_BER_TLV(value_77, "9F4B", RETURN_VALUE)
...
function doTransaction(loop_counter, expected_IAD){
	for(repeat=1; repeat <= loop_counter; repeat++){
...
function ReadAFL(){
	...
	i = 0;
	while(true){	
		tempSFI = AFL.substring(i, i+8);
		if(tempSFI == "") break;
		print("* SFI related Data : " + tempSFI);
		
		startRecord	 = parseInt(tempSFI.substring(2, 4), 16);
		endRecord    = parseInt(tempSFI.substring(4, 6), 16);
		tempSFI      = parseInt(tempSFI.substring(0, 2), 16);
						
		for(record = startRecord; record <= endRecord; record++){
			P1 = to_hex(record, 1)
			P2 = to_hex(tempSFI | 0x04, 1);
			temp = send("00 B2" + P1 + P2 + "00");
			assertSW("9000");
			
			value_70 = lookup_BER_TLV(temp, "70", RETURN_VALUE);
			if(SDAD == "") SDAD = lookup_BER_TLV(value_70 , "9F4B", RETURN_VALUE);		
			if(PKI == "")  PKI  = lookup_BER_TLV(value_70 , "8F", RETURN_VALUE);
			if(CARD == "") CARD = lookup_BER_TLV(value_70 , "9F69", RETURN_VALUE);			
		}
		i += 8;
	}	
	print("* SDAD : " + SDAD);
...
function lookup_SAD() {
	print("\n* Verify Signed Datas in A Signed Record");
		...
		if (SADRecordNumber != 0) {
			for(j = startRecordNumber; j <= endRecordNumber; j++) {
				if (j == SADRecordNumber) {
					...
					while(1) {
						if (recordData.substr(offset,2) == '5F' || recordData.substr(offset,2) == '9F') {
							tagAry[index] = recordData.substr(offset,4);
							offset += 4;
						} else if (recordData.substr(offset,2) != '') {
							tagAry[index] = recordData.substr(offset,2);
							offset += 2;
						} else {break;}
					
						strLength = parseInt('0x' + recordData.substr(offset,2)) * 2;
						offset += 2;
					
						dataAry[index] = recordData.substr(offset,strLength);
						offset += strLength;
...
function check_TLV(tag, len, value) {
	print("\n* Check TLV");
	...
	for(i = 0 ; i < AFL.length ; i += 8) {
		...
		for(j = startRecordNumber; j <= endRecordNumber; j++) {
			...
			recordData = lookup_BER_TLV(recordData, '70', RETURN_VALUE);
			recordData = lookup_BER_TLV(recordData, tag, RETURN_VALUE);
			if (recordData != '') {
				if ((recordData.length/2) == len) {
					if (recordData == value || value == null) {
						test = true;			
					}
				}
			}// if x 3
	...
	if (test == true)
		...
		error('*Fail: TLV is not match.');
}

function check_Data_Length(tag, len) {
	print("\n* Check Data Length");
	...
	for(i = 0 ; i < AFL.length ; i += 8) {
		...
		for(j = startRecordNumber; j <= endRecordNumber; j++) {
			...
			if ((recordData.length/2) > len)
				error('*Fail: Data Length is over.');
			else
				print('*Data Length(byte): ' + recordData.length/2);
		}// for
	}// for
}
...
function check_fDDA(response, IC_PK_MODULUS){
	print('\n*******************************************');
	print('* Read AFL');
	print('*******************************************');
		
	if(AFL == "") error(" there is no AFL");
	...
	while(true){	
		...
		for(record = startRecord; record <= endRecord; record++){
			...
			if(SDAD == "") SDAD = lookup_BER_TLV(value_70 , "9F4B", RETURN_VALUE);		
			if(PKI == "")  PKI  = lookup_BER_TLV(value_70 , "8F", RETURN_VALUE);
			if(CARD_RANDOM_DATA == "") CARD_RANDOM_DATA = lookup_BER_TLV(value_70 , "9F69", RETURN_VALUE);	
			if(CA_PK_INDEX == '') CA_PK_INDEX = lookup_BER_TLV(value_70 , "8F", RETURN_VALUE);
			if(ISSUER_PK_CERTI == '') ISSUER_PK_CERTI = lookup_BER_TLV(value_70 , "90", RETURN_VALUE);
			if(ISSUER_PK_EXPO == '') ISSUER_PK_EXPO = lookup_BER_TLV(value_70 , "9F32", RETURN_VALUE);
			if(IC_PK_CERTI == '') IC_PK_CERTI = lookup_BER_TLV(value_70 , "9F46", RETURN_VALUE);
			if(IC_PK_EXPO == '') IC_PK_EXPO = lookup_BER_TLV(value_70 , "9F47", RETURN_VALUE);
		}
		i += 8;
	}
	
	value_77 = lookup_BER_TLV(response, "77", RETURN_VALUE);
	CTQ  = lookup_BER_TLV(value_77, "9F6C", RETURN_VALUE);
	...
	print("* CTQ : " + CTQ);
	print("* PKI  : " + PKI);
	print("* CARD_RANDOM_DATA : " + CARD_RANDOM_DATA);	
	print("* CA_PK_INDEX : " + CA_PK_INDEX);
	print("* ISSUER_PK_CERTI  : " + ISSUER_PK_CERTI);
	print("* ISSUER_PK_EXPO : " + ISSUER_PK_EXPO);	
	print("* IC_PK_CERTI : " + IC_PK_CERTI);
	print("* IC_PK_EXPO  : " + IC_PK_EXPO);
	
	decrypted_SDAD = decRSAwithPublicKey(SDAD, IC_PK_MODULUS, IC_PK_EXPO);
	print("* Decrypted SDAD : " + decrypted_SDAD);
	...
	print("* Validate Hash Result in decrypted Signed Dynamic Application Data");
	print('************************************************************************');
	hash_input_1 = decrypted_SDAD.substring(2, decrypted_SDAD.length - 42);
	
	if (Terminal_Transaction_Qualifiers.substring(6,8) == '80') { 
		hash_input_2 = Unpredictable_Number + Amount_Authorised + Transaction_Currency_Code + CARD_RANDOM_DATA;
		
		print('* fDDA01');
		print('* Hash Input 1 : ' + hash_input_1);
		print('* Hash Input 2 : ' + hash_input_2);
	} else if (Terminal_Transaction_Qualifiers.substring(6,8) == '00') {
		hash_input_2 = Unpredictable_Number;
	...
	calculated_hash = messageDigest_SHA1(hash_input_1 + hash_input_2).toUpperCase();
	hash_in_SDAD = decrypted_SDAD.substring(decrypted_SDAD.length - 42, decrypted_SDAD.length - 2); 
	
	print("* Pass Hash : " + hash_in_SDAD); 
	print("* Test Hash : " + calculated_hash);
	
	if(calculated_hash != hash_in_SDAD)
		error(" Hash Result Validation...FAIL");
	else 
		print("* Hash Result Validation...PASS");
}

function check_IAD(data, cvr) {
	
	if (data == "" || data == null) error("Issuer Application Data doesn't exist");
	else {
		// check CVN ('01' or '17')
		var iad;
		if (data.substring(4,6) == '01')		iad = IAD_CVN01.substring(0, 8) + cvr + IAD_CVN01.substring(14, IAD_CVN01.length);
		else if (data.substring(4,6) == '11')	iad = IAD_CVN17.substring(0, 8) + cvr + IAD_CVN17.substring(14, IAD_CVN17.length);
		
		print('\n');
		print(IAD_CVN17)
		print(' PASS IAD: ' + iad);
		print(' TEST IAD: ' + data.substring(0, iad.length));
		
		if(data.substring(0,iad.length) == iad) print("\n Correct IAD....PASS");
		else									error("\n Wrong IAD....FAIL");
	}
}

function checkIAD(IAD1, IAD2, length){
	print(Issuer_Application_Data.substring(0,length*2)+" [Card IAD]");
    print(IAD1+" [expected IAD1]");
    print(IAD2+" [expected IAD2]");
    print("\n*************expected IAD1 Parse**************");	
    parse_CVR(IAD1.substring(6,14));
    print("\n*************Card IAD Parse****************");	
    parse_CVR(Issuer_Application_Data.substring(6,14));
	if(Issuer_Application_Data.substring(0,length*2) != IAD1 && Issuer_Application_Data.substring(0,length*2) != IAD2)
		error("Issuer Application Data is wrong.");
}

function compareData(Data1Name, Data1, Data2Name, Data2){
	print(Data1Name + ' : ' + Data1);
	print(Data2Name + ' : ' + Data2);
	if(Data1 != Data2) print('this is not equal');
	else               print('this is equal');	
}

function reset_VLP_Funds(VLP_Funds){
	print('\n* Case : reset VLP Funds');
	...
	//send_PutData("9F77", intToStr(VLP_Funds, 6));
	send_PutData("9F79", intToStr(VLP_Funds, 6)); 
	assertSW("9000");
...
/***20130626 YOUMENG***/
/**********************/
function make_PDOL_Data_new(isEC, isSameCurrency)
{
	...
	while(currentData.length > 0)
	{
		...
		if(currentTag=='DF60'){
			PDOL_Data=PDOL_Data+CAPP_ID_DF60
		}
		...
		if(currentTag=='9F4E'){
			PDOL_Data=PDOL_Data+Merchant_Name_and_Location_9F4E;
...
//	print('*************************************************');
	...
	if((response.substr(0, 2) == "80")&&(getSW() == "9000")){
		AIP = response.substring(4,8);
		AFL = response.substring(8);
	} else if (getSW() == "9000"){
		...
		if(Issuer_Application_Data != "")parse_IAD(Issuer_Application_Data);	
		AC  = lookup_BER_TLV(data, "9F26", RETURN_VALUE);
		Value_9F5D=lookup_BER_TLV(data, "9F5D", RETURN_VALUE);
		Signed_Dynamic_Data=lookup_BER_TLV(data, "9F4B", RETURN_VALUE);
		
    if (CAPP_ID_DF60=="02"){ //Offline Pre-authorized Transaction
    	if (AC!=''){
    		error("Offline Pre-authorized Transaction should not response AC");
    	} 
    } else{
    	validate_AC(AC);
    }
...
{ 
	...
	macSource=xorresult + macSource.substr(16,macSource.length-16)
	...
	mac = create_mac(macSource,Key_in).substr(0,8);
	return mac;
}
...
{
	print('\n*******************************************');
	print('* Read Capp Data');
	print('*******************************************');
	response =send('80B400'+P2_IN+'02'+ID_Number);
	return response;
}
//**********************************************//
//return a string,value is same character,length is the inputlength
//**********************************************//
function PaddingData(Data_c,inputlength)
{
	var data_out="";
	for(var i=1;i<=inputlength;i++)
	{
		data_out=Data_c+data_out;
	}
	return data_out;
}
...
//fDDA
//**********************************************//
function fDDA(fDDA_VN,ICC_PK_Modulus,ICC_PK_Exponent,fDDA_Data)
{
	print('\n*******************************************');
	print('* fDDA');
	print('*******************************************');
	...
	if (fDDA_VN=="01"){
		if(Value_DF61!="") {
			if ((CAPP_ID_DF60=="01") || (CAPP_ID_DF60=="02") || (CAPP_ID_DF60=="03")){
				CARD=CARD.substr(0,14)+Value_DF61;
			}
		}
	  Hash_in = Unpredictable_Number + Amount_Authorised + Transaction_Currency_Code + CARD;		
	}else if(fDDA_VN=="00"){
		Hash_in = Unpredictable_Number;
	}else{
		error("Unknown CVN");
	}
	
  var Recovered_DDA = calc_RSA(ICC_PK_Modulus, ICC_PK_Exponent,fDDA_Data);
  
	print('* Recovered fDDA data: ' + Recovered_DDA);
  print('*******************************************');
	
  if (Recovered_DDA!=""){
    if (Recovered_DDA.substr(0,2)!=RECOVERED_HEAD)	{
  	  error("Recovered Signed Data head error-fDDA");
    }
    if (Recovered_DDA.substr(2,2)!=signed_DATA_FORMAT){
    	error("Signed Data format error-fDDA");
    }
    if (Recovered_DDA.substr((Recovered_DDA.length - 2),2)!=DATA_TRAILER){
    	error("Recovered Signed Data tail error-fDDA");
    }
    
    var ICC_DYNAMIC_DATA_LENGTH = Recovered_DDA.substr(6, 2);
    if (ICC_DYNAMIC_DATA_LENGTH!="03"){
    	error("ICC Dynamic Data Length error-fDDA");
    }
    var ATC_in_fDDA=Recovered_DDA.substr(10, 4);
    
    var ICC_DYNAMIC_DATA_LENGTH_int = to_val(ICC_DYNAMIC_DATA_LENGTH) * 2;  
    var BB_length = ICC_PK_Modulus.length - ICC_DYNAMIC_DATA_LENGTH_int - 50;
    var BB_Recovered = Recovered_DDA.substr((8 + ICC_DYNAMIC_DATA_LENGTH_int),(ICC_PK_Modulus.length - 42 - 8 - ICC_DYNAMIC_DATA_LENGTH_int));
    if (BB_Recovered!=PaddingData("B",BB_length)){
    	error("'BB' padding error-fDDA");
    }
    var HASH_RESULT_DDA = Recovered_DDA.substr((ICC_PK_Modulus.length - 42),40);   
    var HASH_RESULT_DDA_T = messageDigest_SHA1(Recovered_DDA.substr(2,(ICC_PK_Modulus.length - 44)) + Hash_in);
    if (HASH_RESULT_DDA!=HASH_RESULT_DDA_T.toUpperCase()){
    	error("HASH Result error-fDDA");
    }
  }else{
	  error("Recovered fDDA data is empty-fDDA");
  }

}

function intToStr(value, len){
		// Todo for BCD
